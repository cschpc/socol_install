  ...
  while (true)
    {
      ...
      auto yearmon1 = year1 * 100 + month1;

      if (yearmon1 != yearmon2)
        {
          if (Options::cdoVerbose) cdo_print("Process: Year = %4d  Month = %2d", year1, month1);

          auto nrecs2 = cdo_stream_inq_timestep(streamID2, tsID2);
          if (nrecs2 == 0) cdo_abort("Missing year=%4d mon=%2d in %s!", year1, month1, cdo_get_stream_name(1));

          auto vDate2 = taxisInqVdatetime(taxisID2).date;
          int year2, month2, day2;
          cdiDate_decode(vDate2, &year2, &month2, &day2);
          yearmon2 = year2 * 100 + month2;
          if (yearmon1 != yearmon2)
            {
              cdo_abort("Timestep %d in %s has wrong date! Current year=%4d mon=%2d, expected year=%4d mon=%2d", tsID2 + 1,
                        cdo_get_stream_name(1), year2, month2, year1, month1);
            }
