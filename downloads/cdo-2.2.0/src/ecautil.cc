...
void
update_hist(FieldVector2D &field, int nlevels, size_t gridsize, const std::vector<double> &yvals, bool onlyNorth)
{
...
define_mid_of_time(int frequency, int taxisID, int year, int month, int MaxMonths)
{
  ...
  if (frequency == 8)
    {
      ...
      const auto boundmonth = (month + 1 <= MaxMonths) ? month + 1 : 1;
      const auto boundyear = (boundmonth != 1) ? year : year + 1;
  ...
  const auto julianDate1 = julianDate_encode(calendar, vDateTimeBound);
  const auto julianDate2 = julianDate_encode(calendar, vDateTimeBoundP1);

  const auto seconds = julianDate_to_seconds(julianDate_sub(julianDate2, julianDate1)) / 2;
  const auto julianDatem = julianDate_add_seconds(julianDate1, lround(seconds));
  const auto vDateTime = julianDate_decode(calendar, julianDatem);

  taxisDefVdatetime(taxisID, vDateTime);
}

void
adjust_end_date(int nlevels, size_t gridsize, const std::vector<double> &yvals, double missval, int64_t ovdate,
                const FieldVector2D &startDateWithHist, FieldVector2D &endDateWithHist)
{
  const auto ovdateSouth = std::min(cdiEncodeDate(ovdate / 10000, 6, 30), (int) ovdate);
      ...
      for (size_t i = 0; i < gridsize; ++i)
        {
          ...
          if (yvals[i] < 0)
          ...
          else
            {
              ...
              if (DBL_IS_EQUAL(endDateWithHist[0][levelID].vec_d[i], missval)) { endDateWithHist[0][levelID].vec_d[i] = ovdate; }
            }
        }
    }
}
