...
void
timadd(struct dt *dtim, struct dt *dto)
{
  gaint i;
  gaint cont;
...
void
timsub(struct dt *dtim, struct dt *dto)
{
  gaint s1, s2;
...
void
gr2t(gadouble *vals, gadouble gr, struct dt *dtim)
{
  struct dt stim;
  ...
  if (*mnincr > 0.1)
    {
      ...
      /* Do conversion if increment is in months.  Same as for minutes,
         ...
         JMA There is a bug here, and some precision decisions that need
         attention */
    }
  else
    {
      v = *moincr * (gr - 1.0);
      if (v < 0.0)
        dtim->mo = (gaint)(v - 0.9999); /* round (sort of)       */
      else
        dtim->mo = (gaint)(v + 0.0001);
...
gaint
timdif(struct dt *dtim1, struct dt *dtim2)
{
  gaint min1, min2, yr;
  struct dt *temp;
  gaint swap, mo1, mo2;
...
char *
adtprs(char *ch, struct dt *def, struct dt *dtim)
{
  gaint val, flag, i;
  char monam[5];
  ...
  while (*ch >= '0' && *ch <= '9')
    {
      ...
      if (cmpwrd("yr", id))
        dtim->yr = val;
      else if (cmpwrd("mo", id))
        dtim->mo = val;
      else if (cmpwrd("dy", id))
        dtim->dy = val;
      else if (cmpwrd("hr", id))
        dtim->hr = val;
      else if (cmpwrd("mn", id))
        dtim->mn = val;
      else
        {
...
void
getstr(char *ch1, char *ch2, int len)
{
  char *ch;
...
char *
getdbl(char *ch, double *val)
{
  char *pos = NULL;
  double res = ch ? strtod(ch, &pos) : 0.0;
  if (pos == ch)
    {
      return NULL;
    }
...
/* Expand file names prefixed with '^' from data descriptor files */

void
fnmexp(char *out, char *in1, char *in2)
{
...
gadouble
liconv(gadouble *vals, gadouble v)
{
...
gadouble
gr2lev(gadouble *vals, gadouble gr)
{
  gaint i;
...
gadouble
lev2gr(gadouble *vals, gadouble lev)
{
  gaint i, num;
  gadouble gr;
  num = (gaint)(*vals + 0.1);
...
gaint
deflin(char *ch, dsets_t *pfi, gaint dim, gaint flag)
{
  gadouble *vals, v1, v2;
...
gaint
deflev(char *ch, char *rec, dsets_t *pfi, gaint dim)
{
...
gaint
getvnm(struct gavar *pvar, char *mrec)
{
      ...
      /* ensemble name substitution */
      else if (*in == '%' && *(in + 1) == 'e')
        {
          ...
          if (ens == NULL)
            ...
            {
              /* advance through array of ensemble structures, till we reach
               * ensemble 'e' */
              i = 1;
              while (i != e)
                {
...
int
read_gradsdes(char *filename, dsets_t *pfi)
{
      ...
      else if (cmpwrd("format", rec) || cmpwrd("options", rec))
        {
          if ((ch = nxtwrd(rec)) == NULL)
          ...
          else
            {
              while (ch != NULL)
                {
                  if (cmpwrd("sequential", ch))
                    pfi->seqflg = 1;
                  else if (cmpwrd("yrev", ch))
                    pfi->yrflg = 1;
                  else if (cmpwrd("zrev", ch))
                    pfi->zrflg = 1;
                  else if (cmpwrd("template", ch))
                    pfi->tmplat = 1;
                  else if (cmpwrd("flt64", ch))
                    pfi->flt64 = 1;
                  else if (cmpwrd("byteswapped", ch))
                    pfi->bswap = 1;
#if GRIB2
                  else if (cmpwrd("pascals", ch))
                    pfi->pa2mb = 1;
#endif
          ...
          while (1)
            {
              ...
              if (*ch == '*' && (*(ch + 1) == ' ' || *(ch + 1) == '\t'))
                tim2 = -99;
              else if ((ch = intprs(ch, &tim2)) == NULL)
                break;
              if ((ch = nxtwrd(ch)) == NULL) break;
              flag = 0;
              if (pchsub)
          ...
          while (i < pfi->vnum)
            {
              ...
              /* parse the units fields; begin with 0th element for variable
               * units */
              j = 0;
              pvar->nvardims = 0;
              while (1)
  ...
  /* set the global calendar and check if we are trying to change with a new
     file... we do this here to set the calandar for templating */
  if (/*mfcmn.*/ cal365 < 0)
    {
      ...
      if (pfi->calendar != /*mfcmn.*/ cal365)
        {
          ...
          if (/*mfcmn.*/ cal365)
            {
              gaprnt(0, "The calendar is NOW 365 DAYS and you attempted to "
                        "open a standard calendar file\n");
            }
          else
            {
              gaprnt(0, "The calendar is NOW STANDARD and you attempted to "
                        "open a 365-day calendar file\n");
            }
          goto retrn;
        }
    }
