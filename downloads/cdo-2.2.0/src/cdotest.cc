...
using cdoCommand = std::array<std::string, 2>;
...
readNcFile(const char path[], double **vars, int nvars, int nts)
{
  ...
  for (int tsID = 0; tsID < nts; ++tsID)
    {
      const auto nrecs = streamInqTimestep(streamID, tsID);
      assert(nrecs == nvars);
...
createVars(int nvars, int nts)
{
  ...
  for (int i = 0; i < nvars; ++i) vars[i] = &array[i * nts];
