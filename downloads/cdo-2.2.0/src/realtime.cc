...
int
util_cputime(double *user_time, double *system_time)
{
#ifdef __XT3__
...
#if defined(HAVE_SYS_TIMES_H)
  struct tms tbuf;
  if (times(&tbuf) == ((clock_t) -1)) return (-1);
...
void
util_diff_real_time(void *it1, void *it2, double *t)
{
...
util_init_real_time()
{
  ...
  if ((cpuinfo = std::fopen("/proc/cpuinfo", "r")) == NULL)
    {
  ...
  while (!feof(cpuinfo))
    {
      ...
      if (strstr(line, "cpu MHz") != NULL)
        {
          std::sscanf(line, "cpu MHz         : %lf", &freq);
          break;
        }
    }
  if (freq == 0.0)
