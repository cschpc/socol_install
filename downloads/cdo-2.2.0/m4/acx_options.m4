...
AC_ARG_WITH([proj],
            ...
            [AS_CASE(["$with_proj"],
                     [no],[AC_MSG_CHECKING([for proj library])
                           AC_MSG_RESULT([suppressed])],
                     [yes],[AC_CHECK_HEADERS([proj.h])
                            AC_SEARCH_LIBS([proj_create],
                                           [proj],
...
CPPFLAGS="$CPPFLAGS -DCDI_SIZE_TYPE=size_t"
AX_SUBDIRS_CONFIGURE([libcdi], [], [[CPPFLAGS=-DCDI_SIZE_TYPE=size_t]], [], [])
#  ----------------------------------------------------------------------
#  Build a static CDO
